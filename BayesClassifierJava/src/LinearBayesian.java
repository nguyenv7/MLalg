...
public class LinearBayesian extends BayesianMethods {
	ArrayList<Matrix> MeanList = new ArrayList<Matrix>();	
	String[] UniqueTrainLabel = null;
	
	public void Training(ArrayList<ArrayList<Double>> TrainData, ArrayList<String> TrainLabel){
		MeanList.clear();		
		// Unique label
		UniqueTrainLabel = UniqueLabel(TrainLabel);
		// Get list instance of each label
		ArrayList<ArrayList<Integer>> listByLabel = GetElementbyLabel(TrainLabel, UniqueTrainLabel);
	...
	public String Testing(ArrayList<Double> TestData) {
		...
		for (int m = 0 ; m < UniqueTrainLabel.length ; m++){
			for ( int k = m + 1; k < UniqueTrainLabel.length ; k++) {
				double scoreM = MeanDistance(TestData, MeanList.get(m));
				double scoreK = MeanDistance(TestData, MeanList.get(k));
				if ( scoreM < scoreK)
					labelCount[m]++;
				else
					labelCount[k]++;
